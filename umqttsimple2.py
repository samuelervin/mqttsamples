...
class MQTTClient:
            ...
            if res:
                for fd, flag in res:
                    if (not flag & uselect.POLLIN) and (flag & uselect.POLLHUP):
                        raise MQTTException(
                            2 if poller == self.poller_r else 3)
    ...
    def connect(self, clean_session=True):
        ...
        self.poller_r.register(self.sock, uselect.POLLERR |
                               uselect.POLLIN | uselect.POLLHUP)
    ...
    def publish(self, topic, msg, retain=False, qos=0, dup=False):
        ...
        self._write(msg)
        if qos > 0:
            self.rcv_pids[pid] = ticks_add(
                ticks_ms(), self.message_timeout * 1000)
            return pid
